...
for f in ./input/*; do
  ...
  fffmpeg -i "$f" -vf fps=24 -map 0 -c:a libfdk_aac -profile:a aac_he_v2 -b:a 48k -c:s copy -f yuv4mpegpipe -pix_fmt yuv420p -strict -1 |\
...
-o "output/${name}.hevc" - || echo "x265 failed for $f"
